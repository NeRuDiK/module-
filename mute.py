...
@loader.tds
class MeetingMod(loader.Module):
    """Мут."""
    strings = {'name': 'Meeting'}
    ...
    async def ббcmd(self, message):
        ...
        if str(user.id) not in mutes[chatid]:
            ...
            await message.edit("<b>ДаТыЗаебал.</b>")
        else:
            mutes[chatid].remove(str(user.id))
            if len(mutes[chatid]) == 0:
                mutes.pop(chatid)
            self.db.set("Mute", "mutes", mutes)
            await message.edit("<b>Живи сюк.</b>")
    ...
    async def setmutecmd(self, message):
        ...
        except KeyError: return await message.edit("<b>В муте никого!</b>")
