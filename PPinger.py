...
class PPingerMod(loader.Module):
	...
	async def ppingcmd(self, message):
		"""Пингует сам или бот, если вы укажете его имя пользователя с аргументом"""
		...
		if r and (message.entities and isinstance(message.entities[0], mem)):
			try:
				if (await message.client.get_entity(r[0][1:])).bot:
					...
					await message.edit(f'<b>{r[0][1:]}</b> Это не бот')
					return
			except:
				await message.edit(f'<b>{r[0][1:]}</b> Этого пользователя не существует или он не является ботом')
				return
		ping_msg, ping_data, ping_ans = await pping(message, entity, a, t)
		ping = f"""
<code>Средний:</code> {str(mean(ping_data))[0:5]}
<code>Максимальный:</code> {str(max(ping_data))[0:5]}
<code>Минимальный:</code> {str(min(ping_data))[0:5]}"""
		results = 0
		for x in ping_data:
			results += (0 if x >= t * 1000 else 1)
		ok = ("Ваш пинг:" if results == len(ping_data) else f"{results}/{len(ping_data)}")
	...
	async def ppingsetcmd(self, message):
		""".samples <Цифра> - Кол-во попыток пинга
		.timeout <Цифра> - Время ожидания ответа в секундах
